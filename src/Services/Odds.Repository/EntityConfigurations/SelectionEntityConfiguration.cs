...
namespace Odds.Repository.EntityConfigurations
{
    internal class SelectionEntityConfiguration : IEntityTypeConfiguration<Selection>
    {
        public void Configure(EntityTypeBuilder<Selection> builder)
        {
            ...
            builder.Property(s=>s.MarketGuid).HasColumnName("MarketGuid").IsRequired();
            builder.Property(s=>s.Odds).HasColumnName("Odds").IsRequired();
            ...
            builder.HasOne(s => s.Market).WithMany(m => m.Selection).HasForeignKey(p=>p.MarketGuid);
